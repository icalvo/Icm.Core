...
namespace Icm.MathTools
{
	...
	public class StatisticsGenerator
	{
		...
		public readonly LimitedQueue<double> LastFour = new LimitedQueue<double>(4);
		...
		public static double[] Accumulate(IEnumerable<double> freqArray)
		{
		    ...
		    var freqList = freqArray.ToArray();

			Debug.Assert(freqList.Any());

			double[] a = new double[freqList.Length];

			a[0] = freqList[0];

			for (var i = 1; i <= freqList.Length - 1; i++) {
				a[i] = a[i - 1] + freqList[i];
			}
		...
		public int PDFSample(double[] freqArray)
		{
			...
			for (var i = 0; i <= freqArray.GetUpperBound(0); i++) {
				accum += freqArray[i];
				if (U < accum) {
					return i;
				}
			...
			return freqArray.GetUpperBound(0);
		}
		...
		public int CDFSample(double[] accumArray)
		{
			...
			for (var i = 0; i <= accumArray.GetUpperBound(0); i++) {
				if (U < accumArray[i]) {
					return i;
				}
			}
			return accumArray.GetUpperBound(0);
		}
		...
		public double PDFSample(double[] freqArray, double[] values)
		{
			...
			return values[freqArray.GetUpperBound(0)];
		}
		...
		public T CDFSample<T>(double[] accumArray, IEnumerable<T> values)
		{
			...
			return values.ElementAt(accumArray.GetUpperBound(0));
		}

		public void ChangeSeed(int newseed)
		{
			_rng = new Random(newseed % (int.MaxValue - 1));
		}
