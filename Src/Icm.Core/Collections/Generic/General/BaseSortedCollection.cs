...
namespace Icm.Collections.Generic.General
{
	public abstract class BaseSortedCollection<TKey, TValue> : ISortedCollection<TKey, TValue> where TKey : IComparable<TKey>
	{
		...
		private class RangePointIterator : IEnumerator<Tuple<TKey, Nullable2<TValue>>>, IEnumerable<Tuple<TKey, Nullable2<TValue>>>
		{
			...
			public bool MoveNext()
			{
				if (_current == null) {
					...
					var sig = _f.NextKey(_current.Item1);
			...
			public Tuple<TKey, Nullable2<TValue>> Pair(TKey key) {
				if (_f.ContainsKey(key)) {
					return new Tuple<TKey, Nullable2<TValue>>(key, _f[key]);
				} else {
					return new Tuple<TKey, Nullable2<TValue>>(key, default(TValue));
				}
			}
		...
		{
			var it = new PairEnumerator<Tuple<TKey, Nullable2<TValue>>>(new RangePointIterator(this, intStart, intEnd));
			return it;
		}

		public abstract int Count();

		public IEnumerable<Tuple<TKey, Nullable2<TValue>>> PointEnumerable(Nullable2<TKey> intStart, Nullable2<TKey> intEnd)
		{
			return new RangePointIterator(this, intStart, intEnd);
		}

		public IEnumerable<Tuple<TKey, Nullable2<TValue>>> PointEnumerable(Vector2<Nullable2<TKey>> intf)
		{
			return PointEnumerable(intf.Item1, intf.Item2);
		}

		public IEnumerable<Vector2<Tuple<TKey, Nullable2<TValue>>>> IntervalEnumerable()
		{
			return IntervalEnumerable(default(TKey), default(TKey));
		}

		public IEnumerable<Tuple<TKey, Nullable2<TValue>>> PointEnumerable()
		{
			return PointEnumerable(default(TKey), default(TKey));
		}
	}
}
