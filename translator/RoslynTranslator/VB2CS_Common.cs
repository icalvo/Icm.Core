...
namespace Gekka.Roslyn.Translator.VB2CSTrans
{
    static partial class VB2CS
    {
        ...
        static MergeElseIfResult MergeElseIf(VBS.ElseBlockSyntax vbElseBlock, MergeElseIfResult retval = null)
        {
            if (retval == null)
            {
            ...
            if (vbElseBlock != null && vbElseBlock.Statements != null && vbElseBlock.Statements.Count > 0)
            {
                ...
                if (vbStatement.IsKind(VB.SyntaxKind.MultiLineIfBlock))
                {
